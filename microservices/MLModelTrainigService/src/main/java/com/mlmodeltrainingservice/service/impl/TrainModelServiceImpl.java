...
import com.mlmodeltrainingservice.iterator.ListSentenceIterator;
import com.mlmodeltrainingservice.service.TrainModelService;
...
public class TrainModelServiceImpl implements TrainModelService {
	...
	public void trainModelUsingTextFileContent(List<String> sentences) {
		...
		Word2Vec vec = new Word2Vec.Builder().minWordFrequency(1) // Words appearing less than 1 time will be ignored
				...
				.iterate(new ListSentenceIterator(sentences)) // Use the list iterator
	...
	public void trainModelUSingJSonFileContent(JsonNode rootNode) {
		...
		// Build the Word2Vec model
		Word2Vec vec = new Word2Vec.Builder().minWordFrequency(3) // Words appearing less than 1 times will be ignored
				.iterations(10) // Number of iterations over the corpus
				.layerSize(100) // Size of word vectors
				.seed(42) // For reproducibility
				.negativeSample(10).windowSize(2) // Context window size
